...
| 参数 | 固定值 | 说明 |
|------|--------|------|
| 基调 `style` | `dark`（深色科技底） | 统一深色。带小红书贴图的文章可用 `light`（浅蓝），主色仍为品牌蓝 |
| 品牌主色 `palette` | 品牌蓝 `#1f7ae0` / `#5fb8ff` / `#4aa8f5` | 已是脚本默认，**省略 `palette` 即自动品牌蓝**；不要再填 Spring 绿等技术栈色 |
...
> 如个别封面确需换色（极少数），才显式传 `palette` 覆盖；默认场景一律不填，交给品牌蓝默认值。
...
> ⚠️ 技术洋封面默认一律品牌蓝（见顶部「品牌规范」）。下表仅在**极个别**确需与某技术栈强绑定时，才显式传入 `palette` 覆盖；常规文章**不要**用它换色，否则破坏公众号视觉统一。

| 主题 | main | light | accent |
|------|------|-------|--------|
| 技术洋品牌蓝（默认） | `#1f7ae0` | `#5fb8ff` | `#4aa8f5` |
